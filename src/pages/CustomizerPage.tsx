...
import { ProductType, CustomizationArea, Technique, BasicParams, DesignParams } from '../state/types'
...
import { VerticalTabsSidebar, SidebarTabId } from '../components/VerticalTabsSidebar'
...
export const CustomizerPage: React.FC = () => {
  ...
  const [basic, setBasic] = useState<BasicParams>({ size: null, color: null, quantity: null })
  ...
  useEffect(() => {
     if (initialProductData) {
         ...
         if (initialProductData.colors.length > 0) {
             setBasic(prev => ({ ...prev, color: initialProductData.colors[0].name }));
         }
     }
  }, [initialProductData]);
  ...
  function resetForProduct() {
    ...
    setArea([])
    setTechnique('dtg')
    setDesign({ widthMm: null, heightMm: null, colorsCount: null, dpi: null, background: null })
  }

  const { results, cost } = useValidation(product, area, technique, basic, design)
  
  // Resolve the actual product data object to use for visualization
  const activeProductData = (initialProductData && initialProductData.type === product)
    ? initialProductData
    : PRODUCTS.find(p => p.type === product) || PRODUCTS[0];

  // Use mock data price if available, otherwise fallback to calculated base
  const displayBasePrice = activeProductData ? activeProductData.price : cost.base;
  const totalCost = { ...cost, base: displayBasePrice, total: displayBasePrice + cost.techniqueFee + cost.areaFee };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)] bg-background relative overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden relative">
            ...
            <div className="relative z-10 h-full flex shrink-0">
                <VerticalTabsSidebar activeTab={activeTab} onTabChange={setActiveTab}>
                    {activeTab === 'product' && (
                        <>
                            <SidebarSection title="Product Type" required defaultOpen={true}>
                                <ProductSelector value={product} onChange={setProduct} onReset={resetForProduct} />
                            </SidebarSection>
                            <SidebarSection title="Product Specifications" required defaultOpen={true}>
                                <BasicParamsForm value={basic} onChange={setBasic} />
                            </SidebarSection>
                            <SidebarSection title="Print Area" required defaultOpen={true}>
                                <CustomizationAreaSelector product={product} value={area} onChange={setArea} technique={technique} />
                            </SidebarSection>
                            <SidebarSection title="Technique" required defaultOpen={true}>
                                <TechniqueSelector value={technique} onChange={setTechnique} product={product} area={area} />
                            </SidebarSection>
                        </>
                    )}
                    ...
                    {activeTab === 'text' && (
                        <SidebarSection title="Text Editor" required defaultOpen={true}>
                            <TextLayerInput 
                                ...
                                onChange={(updates) => setTextConfig(prev => ({ ...prev, ...updates }))}
                            />
                        </SidebarSection>
                    )}
                    ...
                    {activeTab === 'clipart' && (
                        <SidebarSection title="Clipart Library" required defaultOpen={true}>
                            <ClipartTab />
                        </SidebarSection>
                    )}
                    ...
                    {activeTab === 'ai-assist' && (
                        <SidebarSection title="AI Design Assistant" required defaultOpen={true}>
                            <AIAssistTab 
                                onAutoLayout={() => {
                                    // Mock Auto Layout: Center the content or apply "golden ratio" logic
                                    console.log("AI Auto Layout Applied")
                                }}
                                onApplyFontSuggestion={(font) => setTextConfig(prev => ({ ...prev, font }))}
                                onApplyColorSuggestion={(color) => setTextConfig(prev => ({ ...prev, color }))}
                                results={{
                    ...
                    {activeTab === 'premium' && (
                        <SidebarSection title="Premium Assets" required defaultOpen={true}>
                            <PremiumTab />
                        </SidebarSection>
                    )}
                ...
                {viewMode === '3d' && (
                    <div className="absolute inset-0 z-50 bg-background/50 backdrop-blur-[1px] flex items-center justify-center text-center p-6 cursor-not-allowed">
                        <div className="bg-background/90 p-4 rounded-lg border shadow-lg max-w-[200px]">
                            ...
                            <p className="text-xs text-muted-foreground mt-1">Switch to 2D Design mode to make changes.</p>
                        </div>
                    </div>
                )}
            </div>
        ...
        <FixedBottomBanner 
            results={results} 
            cost={totalCost} 
            onAddToCart={() => console.log('Added to cart', { product, basic, area, technique, design })} 
        />
    </div>
  )
}
