        ...
        <div className="container px-4 md:px-6 mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredTemplates.map((template) => (
              <div
                key={template.id}
                className="group relative bg-background rounded-xl border overflow-hidden hover:shadow-lg transition-all duration-300"
              >
                <div className="aspect-[4/5] relative overflow-hidden bg-muted">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2 p-4">
                    <Link
                      to={`/customizer?template=${template.id}`}
                      className="w-full"
                    >
          ...
          <div className="mt-12 text-center">
            <Link to="/products">
              <Button size="large">View All Templates</Button>
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="container px-4 md:px-6 mx-auto">
          <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            <p className="text-muted-foreground text-lg">
              Built for scale, quality, and creative freedom.
            </p>
          </div>
        ...
        <div className="container px-4 md:px-6 mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {USE_CASES.map((useCase) => (
              <div
                key={useCase.id}
                className="relative group overflow-hidden rounded-2xl aspect-[3/4] md:aspect-[4/5]"
              >
                <img
                  src={useCase.image}
                  alt={useCase.title}
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent p-8 flex flex-col justify-end text-white">
                  ...
                  <Link to="/customizer" className="inline-block">
                    <Button className="bg-white text-black hover:bg-white/90 flex items-center">
                      Start Designing
        ...
        <div className="container px-4 md:px-6 mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {TESTIMONIALS.map((t) => (
              <Card
                key={t.id}
                className="p-8 border bg-background shadow-sm hover:shadow-md transition-shadow"
              >
