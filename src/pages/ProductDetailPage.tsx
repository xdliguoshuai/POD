  ...
  if (!product) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <Link to="/products" className="text-primary hover:underline mt-4 inline-block">Return to collection</Link>
      </div>
    );
  }

  // Update selected color if product changes (safety)
  if (selectedColor && !product.colors.find(c => c.name === selectedColor.name)) {
  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-12">
      ...
      <nav className="flex items-center gap-2 text-sm text-muted-foreground mb-8">
        <Link to="/products" className="hover:text-foreground flex items-center gap-1 transition-colors">
                ...
                {product.images.map((img, i) => (
                    <div 
                        ...
                        className={cn(
                            ...
                            activeImageIndex === i ? "ring-2 ring-primary ring-offset-2" : "opacity-70 hover:opacity-100"
                        )}
            ...
            <div className="space-y-8 pt-6 border-t">
                        ...
                        {product.colors.map((color) => {
                            ...
                            return (
                                <button 
                                    ...
                                    title={color.name}
                                >
                                    {isSelected && <Check size={14} className={cn(color.value === 'white' || color.value === 'bone' || color.value === 'cream' ? "text-black" : "text-white")} />}
                                </button>
                            )
                        })}
                    </div>
                ...
                <div className="flex flex-col sm:flex-row gap-4 pt-2">
                    <Link to={`/customize?product=${product.id}`} className="flex-1">
                        <Button size="lg" className="w-full text-lg h-14 shadow-lg shadow-primary/10">
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground bg-muted/30 p-3 rounded-md border border-border/50">
                    ...
                    <span>Start customizing to validate print areas and get accurate bulk pricing.</span>
                </div>
            </div>
        </div>
      </div>
