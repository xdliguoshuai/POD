  ...
  return (
    <div className="container mx-auto px-4 md:px-6 py-12 md:py-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        <div>
          <h1 className="text-3xl font-bold tracking-tight mb-2">The Collection</h1>
          <p className="text-muted-foreground max-w-[600px]">
            Curated blanks designed for high-end customization. 
            Select a product to begin the design process.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-3 items-center">
            <div className="flex items-center bg-muted/30 p-1 rounded-lg border">
                {(['all', 't-shirt', 'sweatshirt', 'hoodie'] as const).map((type) => (
                    <button
                        key={type}
            ...
            <Card className="h-full border-transparent shadow-none hover:border-border transition-all duration-300 bg-transparent">
              <div className={cn(
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
                    {product.isNew && <Badge className="bg-foreground text-background hover:bg-foreground/90">New</Badge>}
                    {product.isBestSeller && <Badge variant="secondary" className="bg-white/90 backdrop-blur-sm shadow-sm">Best Seller</Badge>}
                </div>
              ...
              <div className="space-y-2 p-4">
                ...
                <div className="flex gap-1.5 pt-1">
                    {product.colors.slice(0, 5).map(c => (
                        <div key={c.name} className="w-3 h-3 rounded-full border border-border/50" style={{ backgroundColor: c.hex }} title={c.name} />
                    ))}
                    {product.colors.length > 5 && <span className="text-[10px] text-muted-foreground">+</span>}
                </div>
              </div>
            </Card>
          </Link>
