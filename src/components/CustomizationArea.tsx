...
export default function CustomizationAreaSelector({ product, value, onChange }: Props) {
  const areas: CustomizationArea[] = ['front', 'back', 'left-sleeve', 'right-sleeve', 'hood']
  return (
    <Card className="shadow-none border-0 bg-transparent">
      <CardHeader className="px-0 pt-0 pb-4">
        <CardTitle className="text-sm font-medium uppercase tracking-wider text-muted-foreground">Print Area (Multi-select)</CardTitle>
      </CardHeader>
      <CardContent className="px-0 flex flex-wrap gap-2">
        {areas.map(a => {
          ...
          const name =
            a === 'front' ? 'Front' :
            a === 'back' ? 'Back' :
            a === 'left-sleeve' ? 'Left Sleeve' :
            a === 'right-sleeve' ? 'Right Sleeve' : 'Hood'
