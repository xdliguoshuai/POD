  ...
  return (
    <div className="space-y-6">
      <div className="flex gap-2 p-1 bg-muted/20 rounded-lg">
          <button className="flex-1 py-1.5 text-xs font-medium rounded-md bg-white shadow-sm">My Designs</button>
          <button className="flex-1 py-1.5 text-xs font-medium rounded-md text-muted-foreground hover:bg-white/50">History</button>
      </div>

      <div className="space-y-4">
        {[1, 2].map((i) => (
            <div key={i} className="flex gap-3 p-3 rounded-lg border bg-background hover:border-primary/50 transition-colors group">
                ...
                <div className="flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-center">
                    <Button size="sm" variant="secondary" className="h-7 text-xs">Load</Button>
                </div>
            </div>
        ))}
      </div>
      ...
      <Button className="w-full gap-2">
          <Save size={16} /> Save Current Design
      </Button>
    </div>
  )
}
