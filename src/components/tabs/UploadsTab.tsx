...
const MOCK_ASSETS: Asset[] = [
  ...
  {
    id: "2",
    url: "https://placehold.co/150x150/jpg",
    name: "Photo_Vintage.jpg",
    type: "jpg",
    size: "3.5MB",
    dpi: 72,
  },
  {
    id: "3",
    url: "https://placehold.co/150x150/svg",
    name: "Icon_Star.svg",
    type: "svg",
    size: "0.5MB",
    dpi: 300,
...
export const UploadsPanel: React.FC<UploadsPanelProps> = ({ onAdd }) => {
  const [assets, setAssets] = useState<Asset[]>(MOCK_ASSETS);
  const [uploadingFile, setUploadingFile] = useState<UploadFile | null>(null);
  const [validationStatus, setValidationStatus] = useState<
    "validating" | "success" | "error" | "warning" | null
  >(null);
  ...
  const handleUploadChange: UploadProps["onChange"] = (info) => {
    ...
    if (
      ...
      // Antd Dragger sometimes treats local as done/error immediately
      validateFile(file);
    }
  };
  ...
  const addToAssets = () => {
    ...
    const newAsset: Asset = {
      ...
      type: uploadingFile.type?.includes("svg")
        ? "svg"
        : uploadingFile.type?.includes("png")
        ? "png"
        : "jpg",
  ...
  const deleteAsset = (id: string) => {
    Modal.confirm({
      title: "Delete Asset?",
      content:
        "This will remove the image from your library. Layers using this image may be affected.",
      okText: "Delete",
      okType: "danger",
      onOk() {
            ...
            {assets.map((asset) => (
              ...
              >
                ...
                <div className="absolute top-1 left-1 flex gap-1">
                  <span className="bg-black/60 text-white text-[9px] px-1 rounded backdrop-blur-sm uppercase">
                    {asset.type}
                  </span>
                  {asset.dpi < 150 && (
                    <span className="bg-orange-500/90 text-white text-[9px] px-1 rounded backdrop-blur-sm">
                      Low DPI
                    </span>
                  )}
                </div>
