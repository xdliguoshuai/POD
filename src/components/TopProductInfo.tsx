...
export function TopProductInfo({ product, basePrice }: Props) {
  const label =
    product === 't-shirt' ? 'Premium Cotton T-Shirt' :
    product === 'sweatshirt' ? 'Heavyweight Crewneck' : 'Signature Hoodie'

  // Mock thumbnails based on type (since Customizer might not have full product object yet)
  const thumbnail = 
    product === 't-shirt' ? 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?auto=format&fit=crop&w=100&q=60' :
    product === 'sweatshirt' ? 'https://images.unsplash.com/photo-1578681994506-b8f463449011?auto=format&fit=crop&w=100&q=60' :
    ...
    <div className="w-full bg-background border-b px-6 py-3 flex items-center justify-between z-20">
        <div className="flex items-center gap-4">
             ...
             <div className="h-10 w-10 bg-muted/20 rounded-md border flex items-center justify-center overflow-hidden relative">
                <img src={thumbnail} alt={label} className="object-cover w-full h-full" />
             </div>
             <div>
